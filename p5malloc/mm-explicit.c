...
static void place(void *bp, size_t asize, int realloc_check);
static void *find_fit(size_t asize);
static void *coalesce(void *bp, size_t asize, int realloc_check);
...
oldsize = GET_SIZE(HDRP(ptr));
  ...
  if(size <= oldsize){
    ...
    char* bp = coalesce(ptr,asize,1);
    size_t prev_alloc = GET_ALLOC(FTRP(PREV_BLKP(ptr)));
    size_t next_alloc = GET_ALLOC(HDRP(NEXT_BLKP(ptr)));
    int next = prev_alloc && !next_alloc;
    if(next){
      place(bp,asize,1);
      return bp;
    }
